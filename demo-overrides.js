...
(() => {
  ...
  if (checkout) {
    ...
    if (locationInput) {
      ...
      locationInput.placeholder = 'Aktifkan Live Location untuk mendapatkan lokasi GPS';
    }

    const oldLocButton = checkout.querySelector('.loc');
    if (oldLocButton) {
      oldLocButton.textContent = '📍 AKTIFKAN LIVE LOCATION';
  ...
  function startLiveLocation() {
    ...
    if (status) status.textContent = 'Meminta izin lokasi dan mencari GPS...';
    if (liveWatchId !== null) navigator.geolocation.clearWatch(liveWatchId);
    liveWatchId = navigator.geolocation.watchPosition(
      pos => {
        const { latitude, longitude, accuracy } = pos.coords;
        const capturedAt = new Date(pos.timestamp || Date.now());
        const mapUrl = `https://maps.google.com/?q=${latitude},${longitude}`;
        liveLocation = { latitude, longitude, accuracy, capturedAt, mapUrl };
        if ($('location')) $('location').value = mapUrl;
        if (status) status.innerHTML = `🟢 <b>Live Location aktif</b><br>Akurasi ±${Math.round(accuracy)} m • Update ${capturedAt.toLocaleTimeString()}`;
        updatePreview();
      },
      err => {
        liveLocation = null;
        if (status) status.textContent = err.code === 1 ? '🔴 Izin lokasi ditolak. Aktifkan izin lokasi browser untuk checkout.' : '🔴 GPS belum tersedia. Coba pindah ke area terbuka lalu ulangi.';
      },
  ...
  function validateCheckout() {
    ...
    if (!name) { alert('Masukkan Nama Pemesan terlebih dahulu.'); $('customerName')?.focus(); return false; }
    if (!liveLocation) { alert('Aktifkan Live Location terlebih dahulu.'); return false; }
    const ageMs = Date.now() - liveLocation.capturedAt.getTime();
    if (ageMs > 120000) { alert('Live Location sudah terlalu lama. Aktifkan/perbarui lokasi kembali.'); return false; }
    return true;
  }
  ...
  const cleanText=root=>{const walker=document.createTreeWalker(root,NodeFilter.SHOW_TEXT);let node;while((node=walker.nextNode()))node.nodeValue=node.nodeValue.replace(/BANDHA\s*RESTO/gi,'DEMO RESTO').replace(/POIPET/gi,'');};
  ...
  style.textContent=`
    ...
    .live-location-status{margin-top:7px;padding:9px 10px;border:1px solid #3b3b3b;border-radius:8px;background:#1b1b1b;color:#ddd;font-size:11px;line-height:1.45;}
    #location[readonly]{opacity:.95;cursor:default;}
